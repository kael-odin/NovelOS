...
# -*- coding: utf-8 -*-
"""
IndexEntityMixin - 实体索引操作

整合自 webnovel-writer/scripts/data_modules/index_entity_mixin.py
"""
...
class IndexEntityMixin:
    def upsert_entity(self, entity, update_metadata: bool = False) -> bool:
        """
        插入或更新实体 (智能合并)
    ...
    def get_entity(self, entity_id: str) -> Optional[Dict]:
        ...
        with self._get_conn() as conn:
            ...
            row = cursor.fetchone()
            if row:
                return self._row_to_dict(row, parse_json_fields=["current_json"])
            return None
    ...
    def get_entities_by_type(
        ...
        with self._get_conn() as conn:
            ...
            return [
                self._row_to_dict(row, parse_json_fields=["current_json"])
    ...
    def record_state_change(self, change) -> int:
        """
        记录状态变化
    ...
    def upsert_relationship(self, rel) -> bool:
        """
        插入或更新关系
