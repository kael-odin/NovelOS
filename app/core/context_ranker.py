...
# -*- coding: utf-8 -*-
"""
Context Ranker - Rank context-pack sections with deterministic heuristics.
"""
...
class ContextRanker:
    ...
    def rank_pack(self, pack: Dict[str, Any], chapter: int) -> Dict[str, Any]:
        ...
        meta["ranker"] = {
            "enabled": True,
            "recency_weight": float(getattr(self.config, 'context_ranker_recency_weight', 0.7)),
            "frequency_weight": float(getattr(self.config, 'context_ranker_frequency_weight', 0.3)),
            "hook_bonus": float(getattr(self.config, 'context_ranker_hook_bonus', 0.2)),
        }
        ranked["meta"] = meta
        return ranked
    ...
    def rank_recent_summaries(self, items: List[Dict[str, Any]], current_chapter: int) -> List[Dict[str, Any]]:
        ...
        for raw in items:
            ...
            hook_bonus = float(getattr(self.config, 'context_ranker_hook_bonus', 0.2)) if self._has_hook_hint(summary) else 0.0
    ...
    def rank_recent_meta(self, items: List[Dict[str, Any]], current_chapter: int) -> List[Dict[str, Any]]:
        ...
        for raw in items:
            ...
            hook_bonus = float(getattr(self.config, 'context_ranker_hook_bonus', 0.2)) if hook else 0.0
            recency = self._recency_score(chapter, current_chapter)
    ...
    def rank_alerts(self, alerts: List[Any], current_chapter: int) -> List[Any]:
        scored = []
        keywords = tuple(getattr(self.config, 'context_ranker_alert_critical_keywords', ['冲突', '矛盾', '错误']))
    ...
    def _combine_score(self, recency: float, frequency: float, bonus: float) -> float:
        return (
            recency * float(getattr(self.config, 'context_ranker_recency_weight', 0.7))
            + frequency * float(getattr(self.config, 'context_ranker_frequency_weight', 0.3))
            + bonus
        )
    ...
    def _length_score(self, text: str) -> float:
        ...
        cap = float(getattr(self.config, 'context_ranker_length_bonus_cap', 0.15))
        return ratio * cap
